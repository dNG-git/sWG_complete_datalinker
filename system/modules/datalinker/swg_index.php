...
case "count":
{
	...
	$g_datalinker_object = ((strpos ($g_eid,"-") === false) ? new direct_datalinker () : new direct_datalinker_uhome ());

	$g_datalinker_array = ($g_datalinker_object ? $g_datalinker_object->get ($g_eid) : NULL);

	if ((!is_array ($g_datalinker_array))||(!strlen ($g_target_url))) { $direct_classes['error_functions']->error_page ("standard","core_datasub_eid_invalid","sWG/#echo(__FILEPATH__)# _a=view_ (#echo(__LINE__)#)"); }
	else
	{
...
case "view":
{
	...
	$direct_cachedata['page_this'] = "m=datalinker&dsd=deid+{$g_eid}++source+".(urlencode ($g_source));
	$direct_cachedata['page_backlink'] = str_replace ("[oid]","deid+{$g_eid}++",$g_source_url);
	$direct_cachedata['page_homelink'] = $direct_cachedata['page_backlink'];
	...
	if (!is_array ($g_datalinker_array)) { $direct_classes['error_functions']->error_page ("standard","core_datasub_eid_invalid","sWG/#echo(__FILEPATH__)# _a=view_ (#echo(__LINE__)#)"); }
	else
	{
		direct_output_related_manager ("datalinker_index_view_".$g_datalinker_array['ddbdatalinker_id'],"pre_module_service_action");
		...
		$g_datalinker_type = $g_datalinker_array['ddbdatalinker_type'];
		$g_datalinker_service = $g_datalinker_object->get_service ($g_datalinker_array['ddbdatalinker_sid']);

		if (isset ($g_datalinker_service['services'][$g_datalinker_type]))
		{
			$g_datalinker_service = $g_datalinker_service['services'][$g_datalinker_type];
			$direct_cachedata['output_object'] = direct_datalinker_iviewer ($g_datalinker_service,$g_datalinker_object);
		...
		if ($g_continue_check)
		{
			...
			direct_output_related_manager ("datalinker_index_view_".$g_datalinker_array['ddbdatalinker_id'],"post_module_service_action");
